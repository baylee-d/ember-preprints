...
function fetchIdFromRelationshipLink(node, relationship) {
    ...
    // accessed under initializedRelationships. May still return
    // undefined if parent, for example, does not exist.
    if (node) {
...
export function getAncestorDescriptor(params/* , hash */) {
    ...
    const parent = (node.get('parent') instanceof EmberObjectProxy) ? node.get('parent.content') : node.get('parent');
